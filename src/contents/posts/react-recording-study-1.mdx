...
Navigator 객체의 이름은 1990년대 넷스케이프(Netscape)의 초기 웹 브라우저인 '네비게이터'에서 유래되었다. 당시에는 브라우저마다 지원하는 기능이 달라서, 웹 개발자들이 방문자의 브라우저 정보를 미리 파악해 호환성을 유지해야 했는데, 이를 **브라우저 스니핑(Browser Sniffing)**이라고 불렀다.
